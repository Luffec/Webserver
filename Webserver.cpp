...
void Webserver::handleRead_(HTTPconnection* client) {
    assert(client);
    extentTime_(client);
    threadpool_->submit(std::bind(&Webserver::onRead_, this, client));//bind绑定类成员函数时，第一个参数表示对象的成员函数的指针，第二个参数表示对象的地址。
...
void Webserver::onProcess_(HTTPconnection* client) 
{
    if(client->handleHTTPConn()) {
        epoller_->modFd(client->getFd(), connectionEvent_ | EPOLLOUT);//EPOLLOUT表示对应的文件描述符可以写
    } 
    else {
        epoller_->modFd(client->getFd(), connectionEvent_ | EPOLLIN);//EPOLLIN表示对应的文件描述符可以读
    }
}
