...
public:
    ...
    //关闭HTTP连接的接口
    void closeHTTPConn();
    //定义处理该HTTP连接的接口，主要为request的解析和response的生成
    bool handleHTTPConn();
    ...
    static bool isET;
    static const char* srcDir;//响应请求的时候需要当前目录的路径这个值的传入
...
private:
    ...
    Buffer readBuffer_;//读缓冲区     HTTP连接读这个缓冲区，内核写入这个缓冲区
    Buffer writeBuffer_;//写缓冲区    HTTP连接写入缓冲区，内核从这个缓冲区读
