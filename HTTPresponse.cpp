...
void HTTPresponse::makeResponse(Buffer& buff){
    /* 判断请求的资源文件 */
    if(stat((srcDir_+path_).data(),&mmFileStat_)<0||S_ISDIR(mmFileStat_.st_mode)){//data()函数将字符串的字符写入数组。它返回一个指向数组的指针，该指针是从字符串到数组的转换获得的
        code_=404;                                                                //int stat(const char *path, struct stat *buf)；
    }                                                                            //st_mode   记录文件类型+文件权限
...
void HTTPresponse::addResponseContent_(Buffer& buff){
    ...
    // 将文件映射到内存提高文件的访问速度 
    // MAP_PRIVATE 建立一个写入时拷贝的私有映射,多进程间数据共享，修改不反应到磁盘实际文件，是一个copy-on-write（写时复制）的映射方式。
